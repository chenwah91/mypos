...
List<paiditems> itemFromJson(String str) => List<paiditems>.from(json.decode(str).map((x) => paiditems.fromJson(x)));
...
class paiditems{
  ...
  String? apr_status;
  String? apr_termtype;
  String? apr_approved_date;

  paiditems(this.apr_billid, this.apr_payerid, this.apr_bill_gbl, this.apr_bill_type, this.apr_totalamt, this.apr_termamt, this.apr_paydate, this.apr_status,this.apr_termtype,this.apr_approved_date);


  paiditems.fromJson(Map<String,dynamic> json){
    ...
    this.apr_approved_date = json['apr_approved_date'];
  }
...
Future<List<paiditems>?> getpaidList(String userID) async {

  List<paiditems> processItems_ = [] ;
  ...
  var data_post = {
    "gePtaidPayment" : "tokenkey",
    "getPaidPayment_accountID" : base64.encode(utf8.encode(userID)).toString(),
  };
  ...
  if(data.statusCode == 200){
    ...
    print("${data.statusCode} < is paiditems statusCode");

    if(data.body != "" && data.body != "noRecord"){

      print(" is paiditems ${data.body}");
      ...
      if(JsonData != "noRecord"){
        ...
        for (var ma in JsonData) {
          ...
          try {
            ...
            paiditems m = paiditems.fromJson(ma);
            print(m.apr_billid);
            processItems_.add(m);
